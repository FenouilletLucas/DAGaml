...
let parse_module stream = parse_module_aux {name=""; params=[]; input=[]; output=[]; wire=[]; assign=[]} stream
;;

let tee stream = stream

let my_verilog = parse_module (my_lexer (tee(Stream.of_channel (open_in file_in))));;
