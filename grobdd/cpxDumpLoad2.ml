...
let dummydump_elem_spx = function S -> "S" | P -> "P" | X(b, i) -> ("X("^(StrUtil.string_of_bool b)^", "^(string_of_int i)^")")

let dummydump_spx (shift, tag, liste) =
	"("^(StrUtil.string_of_bool shift)^
	", {hasS = "^(StrUtil.string_of_bool tag.hasS)^
	"; hasP = "^(StrUtil.string_of_bool tag.hasP)^
	"; maxX = "^(Tools.string_of_option string_of_int tag.maxX)^
	"}, ["^(StrUtil.catmap "; " dummydump_elem_spx liste)^"])"
...
let dummydump_block block =
	"{neg = "^(StrUtil.string_of_bool block.neg)^"; arity = "^(string_of_int block.arity)^"; block = "^(dummydump_block_choice block.block)^"}"
