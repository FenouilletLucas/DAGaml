...
let strload_node = Extra.(StrTree.to_string >> Bitv.L.of_hexa_string >> ZddGops.load_stream ZddGops.binload_node >> ZddGops.node_split)

let strdump_edge = Extra.(ZddGops.dump_stream ZddGops.bindump_edge >> Bitv.L.to_hexa_string >> StrTree.of_string)
let strload_edge = Extra.(StrTree.to_string >> Bitv.L.of_hexa_string >> ZddGops.load_stream ZddGops.binload_edge)
...
let strdump_leaf = StrTree.of_bool
let strload_leaf = Extra.(StrTree.to_bool >> (fun b -> ((0, 0), Utils.Leaf b)))
